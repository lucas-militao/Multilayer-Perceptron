...
#Calcula o gradiente da camada de saída
    def __localGradiente2(self, saidaD, saidaY, entradaI):
...
#Calcular gradiente da camada intermediária
    def __localGradiente1(self, gradienteSaida, pesosSaida, saidaI):
        return np.sum(gradienteSaida*pesosSaida) * (1/(np.cosh(saidaI)**2))
#Calcular os pesos da camada intermediária
    def __atualizarPesos1(self, pesosIntermediarios, taxaDeAprendizagem, gradienteIntermediario, saidaY):
        return pesosIntermediarios + taxaDeAprendizagem*gradienteIntermediario*saidaY
#Calcular o Erro
...
#Realiza o treinamento mas está faltando a parte do backwards, está realizando apenas o feedforward
    def treinamento(self):
        ...
        g2 = np.zeros((np.size(self.amostrasX[:,0]), np.size(self.saidasDesejadasD[0,:])))
        ...
        while(abs(EmAtual - EmAnterior) > self.precisao):
            ...
            for j in range(np.size(g2[:,0])):
                g2[j] = self.__localGradiente2(self.saidasDesejadasD[j], Y2[j], I2[j])
            ...
            for m in range(np.size(g1[:,0])):
                for n in range(np.size(g1[0,:])):
                    g1[m,n] = self.__localGradiente1(g2[m][n], w2[n], I1[m][n])
